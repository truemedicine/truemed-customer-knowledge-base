...
This is one of the most common sources of confusion, and it is worth explaining why it happens.

### Why different products may need separate qualifications

When you order multiple products that fall under different health categories, each one needs its own eligibility check. This is not a Truemed policy quirk; it reflects how IRS Section 213(d) works. To use HSA or FSA funds, a purchase must be medically necessary for a specific health condition. A product that addresses sleep issues draws on different diagnostic criteria than one addressing joint pain or cardiovascular fitness, even if they come from the same cart.

Truemed's provider team evaluates each product category separately to confirm there is a valid medical reason for each purchase.
...
2. **Return to the survey.** Use the survey link from your original email, or request a new link (see "Re-starting a survey" below).
...
Truemed requests health information so our provider team can evaluate whether there is a medically necessary reason for your purchase of a specific product or service. Identifying a specific health condition, rather than a general wellness goal, is a requirement under IRS rules for customers who want to use HSA or FSA funds on certain products.
...
### What happens to your previous progress

Restarting a survey does not carry over any answers from a prior incomplete session. You will need to go through the full survey again from the beginning. Any data you entered previously is not retained.

If your prior session was fully submitted and reviewed, restarting would generate a new, separate review. In that case, contact support before restarting to make sure a new submission is the right next step.
...
- **Check for a follow-up email from Truemed.** If your survey timed out or an issue was detected, Truemed may have already sent a fresh link.
...
- Your order situation is unusual, such as a very large multi-product purchase across several categories.
...
Our team typically responds within one business day. If your order has a time-sensitive fulfillment window, mention that in your message so the team can prioritize accordingly.
